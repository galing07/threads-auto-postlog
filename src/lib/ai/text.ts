...
export async function generateSNSText({
  ...
  const platformRule = isInstagram
    ...
    : isX
      ...
      : `ルール:**必ず${effectiveMaxLength}文字以内（厳守。${effectiveMaxLength}字を1文字でも超えるとThreadsが投稿を拒否するため、超えそうなら削って収めること）**・改行で読みやすく・絵文字適度に使用・ハッシュタグ(#)は付けない（Threadsは複数ハッシュタグに非対応で、#を羅列しても機能しないため）`
