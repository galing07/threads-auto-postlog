...
function DraftCard({
  ...
  // 一度閉じた失敗エラーの本文。同じ内容は再表示しない（しつこく残さない）。
  // 別の新しいエラー本文になれば（!==）再び表示される。
      ...
      <div className="flex gap-0">
        ...
        <div className="flex min-w-0 flex-1 flex-col p-4">
          ...
          {/* 失敗理由（なぜ失敗したか分かるように。再投稿はアクション行の「今すぐ投稿」から）。
              × で閉じられる（しつこく残さない）。閉じた後に別の新しいエラーが出れば再表示する。 */}
          {post.status === 'failed' && post.error_message && post.error_message !== dismissedErrorMsg && (
            <div className="mt-2 flex items-start gap-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-[11px] leading-relaxed text-red-600">
              ...
              <button
                type="button"
                onClick={() => setDismissedErrorMsg(post.error_message)}
