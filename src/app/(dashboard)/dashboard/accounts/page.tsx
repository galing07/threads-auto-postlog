...
const SETUP_GUIDES: Record<SupportedPlatform, SetupGuide> = {
  ...
  instagram: {
    ...
    steps: [
      ...
      '⚠️【初回のみ・最重要】「アプリを作成」→ ユースケースで「その他」を選ぶ → 次の画面でアプリタイプ「ビジネス」を選ぶ（ここで「その他→ビジネス」を選ばないと、後で出てくる「製品」メニューが表示されず、IG投稿の設定ができません。Threads等の専用ユースケースで作ったアプリがこの状態）。アプリ名を入れて作成。1個を全IGアカウントで使い回せます',
      '⚠️【初回のみ】作成後のダッシュボードを下にスクロール →「製品（Products）」の一覧で「Instagram」の「設定」をクリック → セットアップは必ず「Facebookログイン経由（API setup with Facebook login）」を選ぶ（「Instagramログイン経由」を選ぶとページ投稿ができないので注意）。これで instagram_content_publish 等の投稿権限が使えるようになります（＝「製品」項目は消えていません。手順2を「その他→ビジネス」で作れば出てきます）',
