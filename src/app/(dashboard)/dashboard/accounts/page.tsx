...
const SETUP_GUIDES: Record<SupportedPlatform, SetupGuide> = {
  ...
  instagram: {
    ...
    steps: [
      ...
      '【初回のみ・管理者】リダイレクトURLを登録する。場所＝同じ「Instagramログインによる API設定」ページを下にスクロールした番号付きセクション「Instagramビジネスログインを設定する」（「2. アクセストークンを生成する」のさらに下、だいたい「4.」）→「設定する」/「ビジネスログイン設定」ボタン →「リダイレクトURL」欄に、↓の連携パネルの値（コピーボタンでコピー）を完全一致で貼って「保存」。⚠️「Webhooks」でも「ビジネス向けFacebookログイン」でもない（Instagram項目の中）／同セクションの「埋め込みURL」は自動生成の参考表示なのでどこにも貼らなくてOK（リダイレクトURL登録だけで完了）／末尾スラッシュ・前後空白なしで一字一句一致させる',
