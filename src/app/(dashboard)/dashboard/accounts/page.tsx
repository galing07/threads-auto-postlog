...
const SETUP_GUIDES: Record<SupportedPlatform, SetupGuide> = {
  ...
  x: {
    ...
    steps: [
      ...
      '①「コンシューマーキー」の右の「再生成」を押す → 画面に出る API Key を1つ目の欄、API Key Secret を2つ目の欄にコピー（1回しか表示されないので必ずコピー）',
      '②「アクセストークン」の右の「生成する」を押す → 画面に出る Access Token を3つ目の欄、Access Token Secret を4つ目の欄にコピー（こちらも1回限り表示）',
      'この画面で押すボタンは①②の2つだけ。それぞれ2個ずつ・合計4個の値を、↓ の API Key / API Key Secret / Access Token / Access Token Secret に順番に貼る',
      '保存。「X User ID」は空欄でOK（自動取得します）。2個目以降のアカウントも同様に4キーを貼るだけ（App作成は不要）',
      '※「アクセストークン」が「読み取り専用」表示だと投稿できません。先に Settings で「Read and write」にし、その後で必ず「アクセストークン」を再生成すること（古いトークンは読み取り専用のまま）',
    ],
  },
}
