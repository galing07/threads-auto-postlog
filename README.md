...
- **まとめて生成** — 複数投稿を一括生成して下書き（投稿一覧）に保存
- **プレビュー・承認フロー** — 内容を確認してから投稿
- **予約投稿** — 日時を指定して自動投稿（Supabase pg_cron が毎分 `/api/cron/publish-scheduled` を実行）
...
2. **SQL Editor でデータベースを構築**
   - **かんたん（推奨）**: `supabase/full-setup.sql` の全文を貼って実行。schema + 全 migration を連結した**冪等（再実行可）スクリプト**なので、新規 Supabase ならこれ1回でOK
   - 手動で行う場合: まず `supabase/schema.sql` → 続いて `supabase/migrations/` 内の SQL を**ファイル名の日付順**に適用
3. **Storage バケットを作成**
...
| `CRON_SECRET` | 予約投稿の cron 認可トークン。Supabase pg_cron → `/api/cron/publish-scheduled` を `Bearer` で叩く。**予約投稿を使う場合に必須**（pg_cron は本番 URL を叩くためローカルでは発火しません） |
...
│   │   ├── api/                     # API ルート（generate / posts / videos / auth / accounts / cron ...）
...
│   └── types/                       # 型定義
├── supabase/
│   ├── full-setup.sql               # 新規構築用（schema + 全migration 連結・冪等）★これ1回でOK
│   ├── schema.sql                   # ベーススキーマ
